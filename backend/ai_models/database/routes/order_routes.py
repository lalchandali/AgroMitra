...
from database.models.order import Order, OrderStatus, PaymentStatus
from database.models.product import Product, ProductStatus
from database.models.user import User, UserRole
from database.schemas.order_schema import OrderCreate, OrderStatusUpdate, OrderResponse
from database.routes.auth_routes import get_current_user
...
async def place_order(
    ...
    platform_fee  = round(total_amount * PLATFORM_FEE_PERCENT, 2)
    ...
    print(f"  🛒 New order: {new_order.order_id} | ৳{total_amount} | Escrow held")
    return new_order
...
async def update_order_status(
    ...
    farmer_actions = [OrderStatus.confirmed, OrderStatus.ready, OrderStatus.dispatched]
    buyer_actions  = [OrderStatus.delivered, OrderStatus.cancelled]
    ...
    if new_status == OrderStatus.cancelled:
        ...
        if product:
            product.quantity_kg = float(product.quantity_kg) + float(order.quantity_kg)
