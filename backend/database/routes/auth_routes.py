...
def get_current_user(
    ...
    if not user.is_active: raise HTTPException(status_code=403, detail="Account suspended.")
    return user
...
async def register(user_data: UserRegister, db: Session = Depends(get_db)):
    existing = db.query(User).filter(User.mobile_number == user_data.mobile_number).first()
    if existing:
...
async def login(user_data: UserLogin, db: Session = Depends(get_db)):
    ...
    db.commit()
    token_data    = {"user_id": str(user.user_id), "mobile": user.mobile_number, "role": user.role.value}
...
async def refresh_token_route(req: TokenRefresh):
    ...
    new_token = create_access_token({"user_id": payload.get("user_id"), "mobile": payload.get("mobile"), "role": payload.get("role")})
...
async def update_profile(update_data: UserUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    if update_data.name_en:           current_user.name_en = update_data.name_en
    if update_data.name_bn:           current_user.name_bn = update_data.name_bn
    if update_data.district:          current_user.district = update_data.district
    if update_data.profile_photo_url: current_user.profile_photo_url = update_data.profile_photo_url
    db.commit()
...
# ============================================================
#   ADD THIS BLOCK TO: database/routes/auth_routes.py
#   Paste it near the bottom of the file, after the existing
#   routes (e.g. after `deactivate_account` / before the end
#   of the file). It reuses the same imports already present
#   in auth_routes.py (router, get_current_user, User, UserRole,
#   UserResponse, get_db, Session, Depends, HTTPException).
#
#   If any of these are NOT already imported at the top of
#   auth_routes.py, add the missing ones:
#     from typing import List, Optional
#     from database.models.user import User, UserRole
#     from database.schemas.user_schema import UserResponse   # adjust path if different
# ============================================================
